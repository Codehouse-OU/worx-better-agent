...
import ee.codehouse.worxbetter.agent.service.mapper.CurrentStatusMapperImpl;
...
class StatusRequestSchedulerTest {
    ...
    @Mock
    private CurrentStatusMapperImpl currentStatusMapper;

    @InjectMocks
    private StatusRequestScheduler statusRequestScheduler;

    @Captor
    ArgumentCaptor<ee.codehouse.worxbetter.server.model.CurrentStatus> currentStatusArgumentCaptor;
    @Captor
    ArgumentCaptor<QueryLog> queryLogArgumentCaptor;
    ...
    void shouldSendSuccessfulStatus() {
        ...
        verify(mockServerApiClient, times(1)).addStatus(currentStatusArgumentCaptor.capture());
